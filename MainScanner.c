...
nplp_intg errorNumber;				/* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern nplp_intg line; /* Source code line numbers - defined in scanner.c */
extern Token tokenizer(nplp_void);
 ...
 * -------------------------------------------------------------
 */
nplp_void printScannerError(nplp_string fmt, ...);
nplp_void displayScanner(BufferPointer ptrBuffer);
nplp_long getScannerFilesize(nplp_string fname);
